...
(function () {
    angular.module('SSApp').directive('socialProduct', SocialProduct);
    ...
    angular.module('SSApp').directive('multipleEmails', function () {
        return {
            require: 'ngModel',
            link: function (scope, elm, attrs, ctrl) {
                ctrl.$validators.multipleEmails = function (modelValue) {
                    if (ctrl.$isEmpty(modelValue)) {
                        // consider empty models to be valid
                        return true;
                    }

                    return modelValue.split(/[,;]/).every(function (item) {
                        return /^\S+@\S+\.\S+$/.test(item.trim());
                    });
                };
            }
        };
    ...
    angular.module('SSApp').directive('instaSlider', function ($timeout) {
        return {
            restrict: 'A',
            link: function (scope, iElement) {
                scope.$watchGroup(['socialProductImgUrl'], function () {
                    if (typeof scope.socialProductImgUrl !== 'undefined') {
                        $timeout(function () {
                            iElement.removeData('flexslider');
                            iElement.html($('#instaSlData').html());
                            iElement.flexslider({
                                animation: 'slide',
                                controlNav: false,
                                slideshow: false,
    ...
    angular.module('SSApp').directive('twttrSlider', function ($timeout) {
        return {
            restrict: 'A',
            link: function (scope, iElement) {
                scope.$watchGroup(['socialProductImgUrl'], function () {
                    if (typeof scope.socialProductImgUrl !== 'undefined') {
                        $timeout(function () {
                            iElement.removeData('flexslider');
                            iElement.html($('#twitterSlData').html());
                            iElement.flexslider({
                                animation: 'slide',
                                controlNav: false,
                                slideshow: false,
    ...
    SSApp.directive('fbSlider', function ($timeout) {
        return {
            restrict: 'A',
            link: function (scope, iElement) {
                scope.$watchGroup(['socialProductImgUrl'], function () {
