...
SSApp.controller('collectiveController', ['$scope', '$http', 'searchServiceCount', '$route', '$routeParams', '$location', '$cookies', '$interval', '$window', '$timeout', function ($scope, $http, searchServiceCount, $route, $routeParams, $location, $cookies, $interval, $window, $timeout) {

    
	
	
	$scope.$parent.ck_all = true;
    //$scope.$parent.limit = 4;
            ...
            $http.post('/getHashtag', {keyword: $scope.collective_keyword}).then(function (response) {
                ...
                for (var i = 0; i < response.data.length; i++) {
                    ...
                    if (i + 1 == response.data.length) {
                        console.log(arr);
                        $http.post('/productUserMulticatalogID', {mid: arr})
	...
	//	$scope.queryobj.available=true;
		...
		//$scope.$parent.queryobj.offset = 4; // limit
        //$scope.limit = 4;
	   ...
	   $scope.$parent.numPages =0;
        $http.post('/productResult', $scope.queryobj)  //call to retrieve data
            .success(function (data) {
                ...
                angular.element('.products-grid').removeAttr('style');

                setTimeout(function () {

                    console.log('run after 10s')


                    $productsGrid = angular.element('.products-grid');
                ...
                /*$timeout(function () {
                    var parent = angular.element('.products-grid');
