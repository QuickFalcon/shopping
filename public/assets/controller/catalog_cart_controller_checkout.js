...
SSApp.controller('cartControllerCheckout', ['$scope', '$sce', '$http', '$window', '$cookies', '$route', '$location', '$interval','setRequiredfieldCart','$anchorScroll', function ($scope, $sce, $http, $window, $cookies, $route, $location, $interval,setRequiredfieldCart,$anchorScroll) {
    ...
    var temppath;
        ...
        url = '#!/scheckout';
        temppath = {'poth': url, 'label': 'checkout'};
        ...
        $http.post("https://api.twotap.com/v1.0/cart/estimates?public_token=" + $scope.twotap_public_token, para_json).then(function (response3) {
            ...
            if (index > -1) {
                ...
                if (cart_wish == 'cart') {
                    ...
                    if (angular.isDefined($scope.twotap_builtin_cart)) {
                      ...
                      //  $scope.$parent.estimated_move_price_totals = angular.copy(response3.data.estimated_total_prices);
					
					}
                    else {
                        $scope.$parent.estimated_price_totals = {"final_price": "$0.0", "subtotal": "$0.0"};
         ...
         //   else {
                ...
                if (cart_wish == 'cart') {
                    ...
                    $scope.$parent.estimated_price_totals = response3.data.estimated_total_prices;
                    //$scope.$parent.estimated_move_price_totals = angualr.copy(response3.data.estimated_total_prices);
				   ...
				   $scope.twotap_builtin_cart.estimated_total_prices = response3.data.estimated_total_prices;
                    ...
                    // $scope.$parent.saving =$scope.saving+  parseFloat($scope.estimated_price_totals.coupon_value.substring(1),10)
                    // else
           ...
           // }
            ...
            if (cart_wish == 'cart') {
                ...
                $scope.$parent.estimated_price_totals_wish = {"final_price": "$0.0", "subtotal": "$0.0"};
            }
        });
    ...
    $scope.twotapPurchaseConfirm = function () {
        ...
        if ($scope.num_stores > 1) {
            ...
            angular.forEach($scope.twotap_builtin_cart.sites, function (cvalue, ckey) {
                ...
                if (cvalue.card_valid == 1)
                    console.log(cvalue.card_valid);
                else {
            ...
            if ($scope.twotap_builtin_cart.card_valid == 1)
                console.log($scope.twotap_builtin_cart.card_valid);
            else {
                $scope.messageError = "Card Validation Error \n";
            }
    ...
    $scope.twotapCheckoutCart = function (step) {
	    ...
	    $scope.$parent.estimated_price_totals = {};
		 ...
		 } else 
		 {
                ...
                $scope.valid_coupon_store = 0;
                $scope.$parent.fullTotal = 0;
                $scope.$parent.clampedFullTotal = 0;
