angular.module('SSApp').directive('collectiveComponent', ['$http', 'searchServiceCount', '$route', '$routeParams', '$location', '$cookies', '$interval', '$window', '$timeout', function ($http, searchServiceCount, $route, $routeParams, $location, $cookies, $interval, $window, $timeout) {
    return {
        restrict: 'E',
        templateUrl: 'assets/directive/components/collective/collectiveView.html',
        link: function (scope) {
            scope.$parent.ck_all = true;
            scope.similiaritems = [];
            scope.$parent.currentpage = 1;
            var smerchant, catal, pushtrue = true, path = $location.path(), searchParams = {
                path: 'divine'
            };
            var temppath, tempo = [], url1, url2;
            ...
            var arr = [], tempname, tempcode, tempimg, tempbrand, tempcategory, tempprice, temppricesale;
            var tempcurrency, tempmerchant, tempaffiliate_url, tempdiscount = false, temppercentoff = 0, temcol;
            scope.$parent.selected = "";
            var keyStringArr, lowercaseSelectedKeyWord, searchkeyword = "";
                ...
                if (angular.isDefined(scope.searchtermarr) && scope.searchtermarr.length > 0) {
                    ...
                    for (var mx = 0; mx < scope.searchtermarr.length; mx++) {
                        ...
                        for (var k = 0; k < keywordarr.length; k++) {
                            ...
                            if (scope.related_popularsearch.length >= 6) break;
                        }
                    } // mixpanel loop
            ...
            scope.actionCollective = function () {
                ...
                if (scope.optionValue == 'all') {
                    ...
                    $http.post('/getHashtag', {
                        keyword: scope.collective_keyword
                        ...
                        for (var i = 0; i < response.data.length; i++) {
                            ...
                            if (i + 1 == response.data.length) {
                                console.log(arr);
                                $http.post('/productUserMulticatalogID', {
                                    mid: arr
                                    ...
                                    scope.$parent.loaddone = true;
                                    scope.$parent.totalRecordsFound = result.data.product.length; // calculate only first time                             //calculated on first time
                    ...
                    if (scope.collective_keyword.trim() != '') {
                        ...
                        for (var ind = 0; ind < keyStringArr.length; ind++) {
                            ...
                            if ((ind + 1) == keyStringArr.length) {
                                for (var yu = 0; yu < 1000; yu++) { // time waisting loop
                                    if (yu == 999) {
                                        if (scope.color_arr.length > 0) {
                                            for (var keystr = 0; keystr < keyStringArr.length; keystr++) {
                                                ...
                                                if ((keystr + 1) == keyStringArr.length) {
                                                    ...
                                                    if (scope.optionValue != 'hashtag') searchcollectivedatabase();
                                                }
                                            }
                                        } else {
            ...
            function searchcollectivedatabase() {
                ...
                scope.$parent.load_click = 0; // initially befor
                scope.$parent.viewall = 0; //
                scope.$parent.showup = 0;
                scope.$parent.queryobj.start = 0;
                scope.$parent.queryobj.offset = scope.user_offset; // limit
                scope.cproducts = [];
                scope.homeloaded = 1;
                ...
                $http.post('/productResult', scope.queryobj) //call to retrieve data
                    .success(function (data) {
                        ...
                        $http.post('/productResultCount', scope.queryobj).success(function (data) {
                            scope.$parent.totalRecordsFound = data.count; // calculate only first time                             //calculated on first time
                            scope.$parent.numPages = Math.ceil((scope.totalRecordsFound) / (scope.user_offset));
                        }).error(function (cerr) {
                ...
                $timeout(function () {
                    if (pageNo == scope.currentpage && scope.loaddone && pageNo > 1) {
                        if ($(gothere).length) {
                            ...
                            $('html, body').animate({
                                scrollTop: $(gotherePrev).offset().top
                            }, 700);
                        }
                    }
                }, 500);
