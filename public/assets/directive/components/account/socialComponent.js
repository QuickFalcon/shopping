angular.module('SSApp').directive('socialComponent', ['$http', '$route', '$routeParams', '$location', '$interval', 'Upload', 'UserService', 'toastr', function ($http, $route, $routeParams, $location, $interval, Upload, UserService, toastr) {
	return {
        restrict: 'E',
		templateUrl: 'assets/directive/components/account/socialView.html',
		link: function (scope) {
			...
			function opt() {
				...
				$http.post('/getSite', {
					storenamearray: scope.subscribe_stores_coupon
				}).then(function (responseSupportedSite) {
					var jsonObj = {};
					var temp;
			...
			scope.deleteuserporfileMail = function (mkey, mailtype, purchaseId) {
				...
				$http.post('/remove_user_order_email', {
					mailObj: scope.mailObj
			...
			function getcoup(siteidlist) {
				$http.post('/getUserCoupon', {
					subscribelist: scope.subscribe_stores_coupon,
					subscribesiteid: siteidlist
				}).then(function (responseMail) {
					console.log("hi");
