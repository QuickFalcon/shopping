angular.module('SSApp').directive('userComponent', ['$http', '$route', '$routeParams', '$location', 'UserService', '$filter', '$window', 'toastr', function ($http, $route, $routeParams, $location, User, $filter, $window, toastr) {
    return {
        restrict: 'E',
        templateUrl: 'assets/directive/components/user/userView.html',
        link: function (scope) {
            $scope.upclass = 0;
            ...
            function searchuser() {
                ...
                $http.get('/user/gprofile/' + $scope.uid) // call to retrieve data
            ...
            socialPopup.closed();
            if ($scope.loggedin == 1) { searchuser(); }
            // update user data
