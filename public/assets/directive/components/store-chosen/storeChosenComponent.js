angular.module("SSApp").directive("storeChosenComponent", ["$http","$routeParams","$location","categoryNode", function($http, $routeParams, $location, categoryNode) {
    return {
      restrict: "E",
      templateUrl: "assets/directive/components/store-chosen/storeChosenView.html",
      link: function(scope, element, attrs) {
        ...
        if (angular.isDefined($routeParams.merchant) && $routeParams.merchant.trim() != "") {
          ...
          $http.post("/get_storelistSlider", merchants).then(
            function(stores) {
              console.log(merchants);
              scope.currentstore = stores.data;

              scope.$parent.og_image = "http://198.199.109.5:900/" + scope.currentstore[0].domain +".jpg";
                console.log(scope.currentstore[0].domain);
                console.log(scope.$parent.og_image);
            },
            function(err) {
              scope.currentstore = [];
              console.log(err);
              scope.$parent.og_image = "http://198.199.109.5:900/images/logo.jpg";
            }
          );
          ...
          if (scope.firsttime.loading == 0) {
            // search result page first time load
            ...
            scope.$parent.intervalgapGeneratelink(); // create link
          } else scope.$parent.filterCatMerchantBrandList = true;
        ...
        scope.load_more_brand = function() {
          if (scope.busy || scope.brand_start > scope.number_BrandPage) return;
        ...
        scope.searching_within_page = function(keystring, $event) {
          ...
          if (angular.isDefined(keystring) && keystring.trim() != "") {
            ...
            scope.$parent.stopclickInterval();
            scope.$parent.metaxdescription =
              "buy product through ecommerce " + keystring;
        ...
        scope.setIsDiscount = function($event, saletrue) {
          ...
          if (saletrue) scope.$parent.queryobj.sale_offer = saletrue;
          else {
        ...
        function reset_search_string() {
          if (scope.searchforstring.trim() != "") {
            // search string text is not empty

            if (!angular.isDefined(scope.selected)) {
              // selected text is undefined  which lead reuqires reset

              scope.searchforstring = "";
              reset_value_associatedWithString();
            } else if (scope.selected.trim() == "") {
              // selected text is undefined which lead reset
        ...
        function reset_value_associatedWithString() {
          ...
          if (scope.colorFromString) {
            ...
            for (var ci = 0; ci < scope.colorFromStringArr.length; ci++) {
              ...
              if (ci + 1 == scope.colorFromStringArr.length) {
                ...
                } else scope.queryobj.color = scope.$parent.color_arr;
              }
            }
          }
          var tempcat;
          ...
          if (scope.brandFromString) {
            for (
              var brandi = 0;
              brandi < scope.brandFromStringArr.length;
              brandi++
            ) {
        ...
        function make_parent_nodes_empty(level, fullLevelcategory) {
          var temp;
          categoryNode
            .categoryNodeFromId(fullLevelcategory.parent,scope.all_category,scope.selected_categorylist)
            .then(function(response) {
              ...
              if (
                response.selected_categorylist.length !=
                scope.selected_categorylist.length
              ) {
              ...
              categoryNode.categoryNodeFromId(catnode.parent,scope.all_category,scope.selected_categorylist)
                .then(function(response2) {
                  ...
                  categoryNode.categoryNodeFromId(catnode_parent.parent,scope.all_category,scope.selected_categorylist).then(function(response3) {
                      ...
                      if (levely < 2) return;
                      categoryNode
                        .categoryNodeFromId(catnode_parent_grand.parent,scope.all_category,scope.selected_categorylist)
        ...
        scope.catSearchWithSelectPower = function(cate,booldata,fullLevelcategory,level) {
          ...
          if (booldata) {
            ...
            if (index > -1) {
              ...
              scope.$parent.intervalgapGeneratelink();
              if (fullLevelcategory.level > 1)
                // make parent nodes as false when level greater than 1
                make_parent_nodes_empty(
                  fullLevelcategory.level,
                  fullLevelcategory
                );
            } else {
              scope.$parent.loaddone = true;
        ...
        scope.changeBrandList = function(brand, obj, brdObj) {
          ...
          if (obj) {
            ...
            if (index > -1) {
              if (scope.brandFromString) {
                if (scope.brandFromStringArr.indexOf(brand) > -1) {
                  // this brand come from string search
                  scope.$parent.loaddone = true;
                  scope.$parent.brdg[brand] = obj;
                  angular
                    .element("main-search")
                    .scope().vm.checkAllDegnr = false;
                  return;
                }
              }
              scope.$parent.stopclickInterval();
        ...
        scope.setColorWithThis = function(val, color) {
          ...
          if (val) {
            ...
            if (colindex > -1) {
              if (scope.colorFromString) {
                if (scope.colorFromStringArr.indexOf(color) > -1) {
                  // since this color from string we will not delete this color
                  scope.$parent.loaddone = true;
                  return;
                } else {
        ...
        scope.setminprice = function(minprice) {
          ...
          if (scope.minprice > scope.maxprice) {
            ...
            scope.$parent.common_popup_message =
              "minimum price can not be larger than maximum price ";
        ...
        scope.setmaxprice = function(maxprice) {
          ...
          if (scope.minprice > scope.maxprice) {
            scope.$parent.common_popup_header = "Price Range Error";
            scope.$parent.common_popup_message =
              "maximum price can not be smaller than minimum price ";
            $("#commonPopup").modal("show");
            return;
          }
          scope.$parent.loaddone = false;
        ...
        scope.clearFilter = function() {
          ...
          if (angular.isDefined(smerchant)) {
            ...
            scope.$parent.queryobj.merchantList =
            ...
            angular.element("main-search").scope().vm.checkAllShop = false;
            scope.$parent.metaxdescription =
              "buy product through ecommerce of " + smerchant;
            scope.$parent.title = "SS-" + smerchant;
          ...
          scope.$parent.totalMerchants = angular.copy(
            scope.fullMerchantListCount
          );
        ...
        scope.priceFilter = function(pro) {
          ...
          if (pro.price_sale == "") {
            return (
              pro.price >= scope.cost.range[0] &&
              pro.price <= scope.cost.range[1]
            );
          } else {
            return (
              pro.price_sale >= scope.cost.range[0] &&
              pro.price_sale <= scope.cost.range[1]
            );
          }
        };
        // api percent filter
        scope.percentFilter = function(pro) {
          ...
          return (
            pro.percentOff >= scope.percent.range[0] &&
            pro.percentOff <= scope.percent.range[1]
          );
        };
