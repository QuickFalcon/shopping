  ...
  function SignUserDirective($timeout, UserService, $location, $route, toastr) {
    return {
      ...
      link: function(scope) {
        ...
        window.$signUserScope = scope;
        window.$signUserLocation = $location;
        resetForms();
        console.log(vm.signin);
        
        sShoppable.$loginPopupOverlay.find('.modal-content').addClass('hide');
        ...
        $timeout(function() {
          if ($location.search().resetToken) {
            ...
            angular.element('#forgotpassReset').css('right', 0).removeClass('hide');
            sShoppable.$loginPopupOverlay.modal('show');
        ...
        scope.SUBMITTED_forgotPassword = false;
        function enableForgotPasswordButton() {
            scope.SUBMITTED_forgotPassword = false;
        }
        function forgotPassword(form) {
          console.log(vm.signin);
          scope.SUBMITTED_forgotPassword = true;
          sShoppable.$loginPopupOverlay.one('close.login.modal', enableForgotPasswordButton);
          vm.forms.push(form);
          clearPostValidationError(form.email);
          form.email.$validate();
          ...
          UserService
              ...
              .then(function () {
                // localStorage.setItem('TEMP_forgot_password_email', vm.forgot.email);
                // vm.goToStep('#forgotpassSentmail', '#forgotpassEntrMail', 'backward', function () {
                vm.goToStep('#forgotpassSentmail', '', 'backward', function () {
                    scope.SUBMITTED_forgotPassword = false;
                });
                scope.SUBMITTED_forgotPassword = false;
                sShoppable.$loginPopupOverlay.off('close.login.modal');
              })
              .catch(function(err) {
                scope.SUBMITTED_forgotPassword = false;
                sShoppable.$loginPopupOverlay.off('close.login.modal');
        ...
        function resetPassword(form) {
          ...
          UserService
              ...
              .then(function () {
                  vm.goToStep('#forgotpassResetDone', '#forgotpassReset', 'backward', function () {
                        UserService
                          .userSignin({
                              // "email": localStorage.getItem('TEMP_forgot_password_email'),
                              "email": atob($location.search().t),
                              "password": vm.confirm_password
                          })
                          .then(function (data) {
                            ...
                            $location.search({}); // Clear all url parameters
                            scope.$parent.initializeUser(data);
                            // localStorage.removeItem('TEMP_forgot_password_email');
                            // angular.element('html').scope().initializeUser(data);
                            // angular.element('#loginPopup').modal('hide');
        ...
        function goToStep(toStep, fromStep, direction, callback) {
          var $toStep = angular.element(toStep);
          var $fromStep = fromStep ? angular.element(fromStep) : angular.element('#loginPopup .modal-content').not('.hide').not($toStep);
          ...
          scope.overflowHide = true;
          if ($fromStep.length) {
              $fromStep.animate({ right: direction === 'forward' ? '-430px' : '430px' }, 500, function () {
                scope.overflowHide = false;
                $(fromStep).click();
  ...
  angular.module('SSApp').directive('elmCenter', function() {
    return {
      link: function(scope, elem) {
        ...
        elem.parents('.popup-container').click(handler);
        elem.parent().find('input').on('input', function (event) {
