...
!(function($){
	...
	*/
	...
	var sShoppable = function() {
		this.findAndInitCustomSelects();
	...
	sShoppable.prototype.init = function() {

		return this.initCustomScrollbar();
		return this.ssSignUp();
		return this.ssLogIn();
	...
	sShoppable.prototype.ssLogIn = function() {
	...
	sShoppable.prototype.ssSignUp = function() {
    ...
    * Custom select
    */
    sShoppable.prototype.findAndInitCustomSelects = function() {
	...
	sShoppable.prototype.initCustomScrollbar = function() {
...
*/
		...
		$('#searchCategory').find('>li').each(function(i){
			$(this).click(function(){
				...
				if( $(this).hasClass('active')) return false
					...
					if ($(this).is('#searchCategory li:first')) {
						checkbxChecked($('#searchResult >div').find('label'))
					}
				}
			})
		})
		...
		$('#searchEng').on('click touchstart',function (e) {
			e.stopPropagation();
...
/*			var shortpress = false;
			var startTime, endTime;
				...
				$('.main_nav >li.choose_cat').on('touchstart', function () {
			        ...
			        var thisItem = $(this);
			        pressTimer = window.setTimeout(function() {
				        thisIcon.addClass('zooming');
			    	},1000);

			    }).on('touchend', function (e) {
			    	e.stopPropagation();
			        endTime = new Date().getTime();
			        shortpress = (endTime - startTime < 1000) ? true : false;
...
*/
			...
			var startTime, endTime;
	 		$('.product-info').each(function() {
				...
				$(this).on('touchstart', function (e) {
			        ...
			        pressTimer = window.setTimeout(function() {
				        thisIcon.addClass('zooming');
				        $('.product-info .shared_icons li').not(thisIcon).removeClass('zooming');
				        closeSocialPup();
			    	},1000);
			        ...
			        if(shortpress){
			        	$('.product-info .shared_icons li').removeClass('zooming');
		...
		$('.s_alphabet_item ul li input[type="checkbox"]').change(function() {
    		var k = 0;
    		var j = 0;
			var selectedStore = '';
