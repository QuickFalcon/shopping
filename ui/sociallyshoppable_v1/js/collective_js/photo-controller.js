...
(function(window, $, undefined) {
	...
	window.PhotoListController = function() {
			...
			$.ajax({
				...
				success: function(response) {
					...
					if (response && response.status == 'success') {
						...
						$loadingEl.find('.load_more').html('Load More')
						$loadingEl.find('.load_more').removeClass('loading_dot')
		...
		var bindEvent = function() {
			...
			$(document).on('click','.cltv_loading',function() {
				that.getData();
				$(this).find('.load_more').html('Loading')
				$(this).find('.load_more').addClass('loading_dot')
	...
	photoListControl.init($('.products-grid'), $('.cltv_loading'), $('.photo-template .products-grid__item'), dataUrl);
