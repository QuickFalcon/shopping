...
$lastHours  = new UTCDateTime(strtotime('-24hours')*1000);
...
if (count($resStore) > 0) {
    ...
    $jobRunning = $modelCronJobs->count(
        [
            '$or' => [
                ...
                ['job_done' => ['$exists' => true, '$ne' => true], 'job_type' => 'facet'],
            ]
        ]
    );
    ...
    mail('mainuljs@gmail.com', 'Facet Cron', $emailBody, $mailheaders);
}