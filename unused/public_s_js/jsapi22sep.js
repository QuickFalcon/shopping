...
(function($){
	$(function(){
					...
					$(this).click(function(){
						...
						if( $(this).hasClass('active')) return false
							...
							if(socialPopup.length){
								...
								if($(this).parents('#prSocialPopup').find('.tab_head >li').eq(i).is(':last-child')){
									$(this).parents('#prSocialPopup').addClass('uploadThumb');
									sShoppable.slider.uploadThumbSlider();
								}else{
									$(this).parents('#prSocialPopup').removeClass('uploadThumb');
								} 
		...
		sShoppable.slider = {
			...
			uploadThumbSlider : function () {
				// Uploaded thumb slider
				$('.upload-thumb-list').flexslider({
					//asNavFor: '.thumb_viewer_wrap',
					selector: '.ut_list >li',
					animation : "slide",
					controlNav : false,
					animationSpeed : 400,
		...
		sShoppable.socialPopup = {
			...
			init: function() {
			  sShoppable.socialPopup.trigger();
			},
							...
							$(this).on('click touchstart', function (e){
								...
								if( $(this).hasClass('active')) return false
									...
									if(!$(this).parents('.n_thumbnail').length){
										if($('#prSocialPopup').find('.tab_head li').eq(i).is(':last-child')){
											$('#prSocialPopup').addClass('uploadThumb');
											sShoppable.slider.uploadThumbSlider();
										}else{
											socialPopup.removeClass('uploadThumb');
										} 
		...
		var socailPupInModal = false;
		$('#qViewShare .shared_icons li').each(function(i){
			$(this).click(function(e){
				e.stopPropagation();
				socailPupInModal = true;
		...
		$('.socail_tab_wrap').each(function() {
 			sShoppable.Tab($(this).find('.socialTabHead'), $(this).find('.socialTabContentWrp'));
		})
		...
		$('.rateis').each(function(){
			if (screen.width > 767) {
				...
				$('.rateis.showRate').on('touchstart',function(){
				...
				function highlightHoveredObject(x, y) {
					$('.rateis.mainRateBox').each(function(){
...
*/
		...
		$('.logo_list').each(function() {
			...
			if (screen.width > 767) {
				...
				storeNo > 3 ? $(this).addClass('threeDot') : $(this).removeClass('threeDot')
			}
	...
	function handleCustomSelectIssues( $customSelectInput, customInputItemNo){
		...
		$customSelectDiv.find('li').each(function(i){
			...
			$this_li.click(function(e){
				...
				$customSelectInput.find('option:eq('+i+')').change()
