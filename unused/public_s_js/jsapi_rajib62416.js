(function($){
	$(function(){
		...
		$('.main_nav > li').each(function(){
			...
			if(	screen.width > 768 )$('.subnav_wrap ul li.searchIn_nav').remove();
		})
		...
		if($('.carousel_store1').length){
			$('.carouselStage_title h1').hover(function(){
				$('.stage-carousel-popup').addClass('open');
			},function(){
				if (!$('#stageCarouselPopup').is(":hover")) {
					$('.stage-carousel-popup').removeClass('open')
				}
				
			})

			$('.stage-carousel-popup').mouseleave(function() {
				$('.stage-carousel-popup').removeClass('open')
			})
			$('.stage-carousel-popup .close-popup').click(function(){$('.stage-carousel-popup').removeClass('open')})
		}
		...
		$('.subscribeCoupon').click(function(e) {
			$('#storeSubscribed').modal('show')
		})
