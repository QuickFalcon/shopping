...
UserApp.factory('prosperentUrlGen',function prosperentUrlGen($http, $q,$location,$cookies) { 
       ...
       // if user click on menu item
        ...
        // that means we do not find any mapped category
                ...
                if (scope.menuClicked == "women") {
                    ...
                    if (mcount == 0) {
                        scope.select_category = "*shoe*|*sneakers*|*boots*|*slippers|*trainers*|*shoes*";
                        mcount = 1;
                    }
                    else
                        scope.select_category = "*shoe*|*sneakers*|*boots*|*slippers*|*trainers*|*shoes*|" + scope.select_category;

                    scope.similiarpro = "*shoe*|*sneakers*|*boots*|*slippers|*trainers*|*shoes*";
                }
                else if (scope.menuClicked == "accessories") {
                    
                     
                    if (mcount == 0) {
                        scope.select_category =  scope.urlencoding("*accessory*")+"|"+scope.urlencoding("*accessories*");
                        mcount = 1;
                    }
                    else
                        scope.select_category = scope.urlencoding("*accessory*")+"|"+scope.urlencoding("*accessories*")+"|" + scope.select_category;


                    scope.similiarpro = scope.urlencoding("*accessory*")+"|"+scope.urlencoding("*accessories*");
                }
                else if (scope.menuClicked == "beauty") {
                    if (mcount == 0) {
                        scope.select_category = scope.urlencoding("*beauty*")+"|"+scope.urlencoding("*jewelry*")+"|"+scope.urlencoding("*skin*");
                        mcount = 1;
                    }
                    else
                        scope.select_category = scope.urlencoding("*beauty*")+"|"+scope.urlencoding("*jewelry*")+"|"+scope.urlencoding("*skin*")+"|" + scope.select_category;


                    scope.similiarpro = scope.urlencoding("*beauty*")+"|"+scope.urlencoding("*jewelry*")+"|"+scope.urlencoding("*skin*");
                }
                    ...
                    if (mcount == 0) {
                        ...
                        scope.select_category = scope.urlencoding("*home*")+"|"+scope.urlencoding("*room*")+"|"+scope.urlencoding("*furniture*")+"|"+scope.urlencoding("*kitchen*")+"|"+scope.urlencoding("*light*")+"|"+scope.urlencoding("*curtain*")+"|"+scope.urlencoding("*Utensils*")+"|"+scope.urlencoding("*pan*")+"|"+scope.urlencoding("*pot*")+"|"+scope.urlencoding("*garden*");
                        mcount = 1;
                    }
                    else
                        scope.select_category = scope.urlencoding("*home*")+"|"+scope.urlencoding("*room*")+"|"+scope.urlencoding("*furniture*")+"|"+scope.urlencoding("*kitchen*")+"|"+scope.urlencoding("*light*")+"|"+scope.urlencoding("*curtain*")+"|"+scope.urlencoding("*Utensils*")+"|"+scope.urlencoding("*pan*")+"|"+scope.urlencoding("*pot*")+"|"+scope.urlencoding("*garden*")+"|" + scope.select_category;


                    scope.similiarpro =   scope.urlencoding("*home*")+"|"+scope.urlencoding("*room*")+"|"+scope.urlencoding("*furniture*")+"|"+scope.urlencoding("*kitchen*")+"|"+scope.urlencoding("*light*")+"|"+scope.urlencoding("*curtain*")+"|"+scope.urlencoding("*Utensils*")+"|"+scope.urlencoding("*pan*")+"|"+scope.urlencoding("*pot*")+"|"+scope.urlencoding("*garden*");
                }
                    ...
                    if(temp_cat.trim()!="" && angular.isDefined(scope.mapped_cat[temp_cat]) && scope.mapped_cat[temp_cat].length>0) {
                            ...
                            for(map_count=0;map_count<temp_map.length;map_count++) {
                                ...
                                if(mcount==0)                              // with out anything will happen for first category
                                    ...
                                    scope.select_category = scope.select_category  +"|"+scope.urlencoding(tty);
    ...
    var count_color = scope.color_arr.length;
	   ...
	   if(scope.oc_work) 
        ...
        ocas = "|"+astrickEncd + ocas1;

		wco = scope.urlencoding("wear to work");
       
	    gw1 = wco+ astrickEncd + "|"+ astrickEncd + scope.urlencoding("uniform*");
		...
		ov1 =  scope.urlencoding("vacation*")+"|"+ scope.urlencoding("*holiday*");

        ov = "|"+ astrickEncd +  ov1;

       // var ocos ="|*valentine*|*hallowen*|*christmas*|*marriage*|*engagement*|*victory*|*eid*|*shabbat*|*Holi*|*Diwali*|*Magha*";
        
        ocos1 =scope.urlencoding("valentine*")+"|"+scope.urlencoding("*hallowen*")+"|"+ scope.urlencoding("*christmas*")+"|"+scope.urlencoding("*marriage*")+"|"+scope.urlencoding("*engagement*")+"|"+scope.urlencoding("*victory*")+"|"+ scope.urlencoding("*eid*") +"|"+ scope.urlencoding("*shabbat*")+"|"+ scope.urlencoding("*Holi*") +"|"+ scope.urlencoding("*Diwali*")+"|"+scope.urlencoding("*Magha*");
        ocos = "|"+ astrickEncd + ocos1;
		ocov = ocos;
		ocov1 = ocos1; // clear 
        ...
        if (scope.color_arr.length>0 && count_ocassion>0) {
		     ...
		     for(var colindex=0;colindex<scope.color_arr.length;colindex++) {
                if(colindex>0) {
					scope.select_color = scope.select_color + "|"+scope.urlencoding("*"+scope.color_arr[colindex] +"*");
				    tempcol = scope.urlencoding("*"+scope.color_arr[colindex] +"*");
                    ocas1 =ocas1 +"|" + tempcol + scope.urlencoding("casual*");
					ov1 = ov1 +"|" + tempcol +scope.urlencoding("vacation*")+"|"+tempcol+ scope.urlencoding("holiday*");
					gw1 = gw1 +"|" + tempcol + wco+ astrickEncd + "|"+ tempcol +  scope.urlencoding("uniform*");
                  ...
                  tempcol = scope.urlencoding("*"+scope.color_arr[colindex] +"*");
				   ocas1  = tempcol + scope.urlencoding("casual*");
					ov1 =  tempcol +scope.urlencoding("vacation*")+"|"+tempcol+ scope.urlencoding("holiday*");
					gw1 = tempcol + wco+ astrickEncd + "|"+ tempcol +  scope.urlencoding("uniform*");
					ocos1 =tempcol + scope.urlencoding("valentine*")+"|"+tempcol+scope.urlencoding("hallowen*")+"|"+tempcol+ scope.urlencoding("christmas*")+"|"+tempcol+scope.urlencoding("marriage*")+"|"+tempcol+scope.urlencoding("engagement*")+"|"+tempcol+scope.urlencoding("victory*")+"|"+tempcol+ scope.urlencoding("eid*") +"|"+ tempcol+scope.urlencoding("shabbat*")+"|"+ tempcol+ scope.urlencoding("Holi*") +"|"+tempcol+ scope.urlencoding("Diwali*")+"|"+tempcol+scope.urlencoding("Magha*");
