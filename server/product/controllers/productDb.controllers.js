...
//categories_arr.0
//ss_tap_Product search in this collection
exports.productResult = function (req, res, next) {
    var searchquery = {},
        // selected_str = req.params.search,
    ...
    if (req.body.color) {
        if (req.body.color.length > 0) {
            ...
            for (var p = 0; p < colarr.length; p += 1) {
                ...
                colarrUpperCase[p] = colarr[p].toUpperCase();
                colarrLowerCase[p] = colarr[p].toLowerCase();
                for(var x = 0; x < colarr[p].length; x++){
                    if(x == 0)
                        colarrCaseInsensetive[p] = colarr[p].charAt(0).toUpperCase();
                    else
                        colarrCaseInsensetive[p] += colarr[p].charAt(x).toLowerCase();
                }
            }
            searchquery.color = { $in: [colarrCaseInsensetive, colarrUpperCase, colarrLowerCase] };
            //colboolean = true;
        }
    }
    ...
    var offset = req.body.offset, start = offset * req.body.start, lquery;
    ...
    lquery.exec(function (err, data, next) {   // run query to find data result
        if (err) {
            console.log(err);
            res.end();
            return next(err);
...
// count number of returned product
...
exports.productResultByCatalogId = function (req, res, next) {
    ...
    var lquery = ProductProsperent.find(searchquery);
...
exports.ProductProsperentSocialUpdate = function (req, res, next) {
    var cid = req.body.catalogId, update_fields = req.body.socialFields;
    ProductProsperent
        .update({ catalogId: cid }, { $set: update_fields }, { new: true })
...
exports.ProductProsperentUpdate = function (req, res, next) {
    var cid = req.body.catalogId, update_fields = { available: req.body.available, last_checked: new Date() };
    if (req.body.price) {
...
exports.productSuggestion = function (req, res, next) {
    ...
    var searchquery = {},
        sortingorder;
    if (req.body.keyword) {
        var reqVar = req.body.keyword, reqVar2 = new RegExp(reqVar, 'i');
...
exports.productcount = function (req, res) {
    ...
    if (req.body.keyword) {
        ...
        var skey = searchkey.replace(/-/g, ' ');  // replace to remove negative form mongodb because - means negate a word
        searchquery = { $text: { $search: skey } };
    }
    if (req.body.designer) {
        searchquery.designer = req.body.designer;
    }
    if (req.body.brand) {
        searchquery.brand = req.body.brand;
    }
    if (req.body.merchant) {
        searchquery.merchant = req.body.merchant;
    }
    if (req.body.color) {
    ...
    var query = Product.find(global.searchedObject, '_id').count();
    // query = Product.find(searchquery,'_id').count();
    query.exec(function (err, result) {
        if (!err) {
            res.json({ 'count': result });
        } else {
...
exports.productResultGroupBrand = function (req, res, next) {
    ...
    // var selected_str=req.params.search;
    var searchquery = {}, occ = '';
    var recordsess = req.session, sortingorder;
    if (req.body.keyword) {
        var searchkey = req.body.keyword;
    ...
    if (req.body.categoryList && req.body.categoryListId) {
        ...
        searchquery.$or = [{ category: { $in: myarr } }, { categoryId: { $in: catArr } }];
    ...
    var offsetBrd = req.body.limit, query;
                    ...
                    if (result.length > 0) {
                        ...
                        for (var t = 0; t < result.length; t += 1) {
                            if (result[t]._id.brand != null) {
                                ...
                                if (
                                    firstalphabet == 'a' || firstalphabet == 'b' || firstalphabet == 'c' || firstalphabet == 'd' ||
                                    firstalphabet == 'e' || firstalphabet == 'f' || firstalphabet == 'g' || firstalphabet == 'h' ||
                                    firstalphabet == 'i' || firstalphabet == 'j' || firstalphabet == 'k' || firstalphabet == 'l' ||
                                    firstalphabet == 'm' || firstalphabet == 'n' || firstalphabet == 'o' || firstalphabet == 'p' ||
                                    firstalphabet == 'q' || firstalphabet == 'r' || firstalphabet == 's' || firstalphabet == 't' ||
                                    firstalphabet == 'u' || firstalphabet == 'v' || firstalphabet == 'w' || firstalphabet == 'x' ||
...
exports.productResultGroupMerchant = function (req, res, next) {
    ...
    console.log(offsetMer);
    if (offsetMer == 6) {
        var query = ProductProsperent.aggregate([{ $match: searchquery }, { $group: { _id: { merchant: '$merchant' } } }, { $sort: { _id: 1 } }, { $limit: offsetMer }]);
    } else {
    ...
    // var query = ProductProsperent.distinct('merchant', searchquery);
    // query.exec(function (err, result) {
    //     var temp_merchant;
    //     if (!err) {
    //         async.waterfall([
    ...
    //                 var resultSet = [];
    //                 for (var t = 0; t < result.length; t += 1) {
    //                     temp_merchant = { 'merchant': result[t] };
    //                     resultSet.push(temp_merchant);
    //                 }
    //                 callback(null, resultSet);
    //             }
...
exports.productslistfullpath = function (req, res) {
    // var selected_str=req.params.search;
    //  var fullpath = req.params.fullpath;
    //  var s2 = "&filterMerchant";
    // use mongoose to get all todos in the database
    Product.find({}, function (err, productdata, next) {
        // if there is an error retrieving, send the error. nothing after res.send(err) will execute
        if (err) { return next(err); }
        res.send(productdata); // return all todos in JSON format
    });
};
...
*/
exports.productsync = function (req, res, next) {
    Product
        .find({})
        .sort('catalogId')
        .select({
...
exports.productsingleuser = function (req, res) {
    Product.find({ catalogId: req.params.item }, function (err, data) {
        res.send(data);
    });
};
...
exports.productSave = function (req, res, next) {
    var tempproduct = new Product({
        catalogId: req.body.catalogId,
        productId: req.body.productId,
        keyword: req.body.keyword,
        availability: req.body.availability,
        prosper_availability: req.body.prosper_availability, // new field prosper availability
        keywords: req.body.keywords,
        gender: req.body.gender,
        category: req.body.category,
        SScategory: req.body.SS_category,
        ...
        sale_offer: req.body.sale_offer,
        sales: req.body.price_sale,
        ...
        occasion_casual: req.body.occasion_casual,
        occasion_vacation: req.body.occasion_vacation,
        occasion_work: req.body.occasion_work,
        occasion_other: req.body.occasion_other,
        created_at: req.body.insert_date
    });
    tempproduct.save(function (err) {
        if (err) { return next(err); }
        res.send('success');
    });
};
// ////////////////////// ADMIN PRODUCT CRUD
exports.productSaveMultiple = function (req, res, next) {
    var tempproduct = new Product({
        catalogId: req.body.catalogId,
        productId: req.body.productId,
        keyword: req.body.keyword,
...
exports.likeUpdate = function (req, res) {
    Product.update(
        { catalogId: req.body.catalogId },
        {
            catalogId: req.body.catalogId,
        ...
        .exec(function (err, results) {
            if (err || results < 1) {
                res.json(404, { msg: 'Failed to update top women store' });
            } else {
                res.json({ msg: 'Customer top ten Updated' });
            }
        });
};